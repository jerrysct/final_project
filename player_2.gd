...
func _ready() -> void:
	...
	if skill_indicator: skill_indicator.top_level = true; skill_indicator.z_index = 50
	if release_burst_particles: release_burst_particles.top_level = true; release_burst_particles.z_index = 51
	if head_bullet_display: head_bullet_display.top_level = true; head_bullet_display.z_index = 52
...
func _get_hp_text() -> String: return str(int(current_hp)) + " / " + str(int(Playerdata_Globle.max_hp))
func _get_stamina_text() -> String: return str(int(current_stamina)) + " / " + str(int(Playerdata_Globle.max_stamina))
func _get_mp_text() -> String: return str(int(current_mp)) + " / " + str(int(Playerdata_Globle.max_mp))

func _update_player_ui() -> void:
	if health_bar: health_bar.max_value = Playerdata_Globle.max_hp; health_bar.value = current_hp
	if hp_label: hp_label.text = _get_hp_text()
	if stamina_bar: stamina_bar.max_value = Playerdata_Globle.max_stamina; stamina_bar.value = current_stamina
	if stamina_label: stamina_label.text = _get_stamina_text()
	if mp_bar: mp_bar.max_value = Playerdata_Globle.max_mp; mp_bar.value = current_mp
