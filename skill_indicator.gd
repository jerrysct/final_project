...
# 接收玩家腳本傳來的大小，預設值對應你原本的設定 (100與200)
...
func _draw() -> void:
	...
	if is_parry_preparing:
		if radius > inner_radius:
			...
			# 畫邊緣線，讓視覺更清晰
			draw_arc(Vector2.ZERO, radius, start_angle, end_angle, point_count, Color(1.0, 1.0, 1.0, 0.8), 3.0, true)        # 外圈線
			draw_arc(Vector2.ZERO, inner_radius, start_angle, end_angle, point_count, Color(1.0, 1.0, 1.0, 0.8), 3.0, true)  # 內圈線
