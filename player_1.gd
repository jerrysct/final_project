...
# 將原本的 ChargeBar 變數改為 mp_bar，為了避免你場景報錯，這裡維持原節點路徑名 ChargeBar
# 建議有空可以把場景樹裡的節點名稱也改成 MpBar，並將下方路徑同步修改
@onready var mp_bar: ProgressBar = $CanvasLayer2/ChargeBar
@onready var mp_label: Label = $CanvasLayer2/ChargeBar/Label   
...
@export var skill_mp_cost: float = 20.0  # 新增：施放技能消耗的魔力
...
func _ready() -> void:
	...
	# 初始化魔力 (請確保 Playerdata_Globle 中有 max_mp 變數)
	current_mp = Playerdata_Globle.max_mp if "max_mp" in Playerdata_Globle else 100.0
	...
	# 魔力條設定 (原蓄力條)
	if mp_bar:
		mp_bar.max_value = Playerdata_Globle.max_mp if "max_mp" in Playerdata_Globle else 100.0
		mp_bar.value = current_mp
		mp_bar.visible = true # 保持常駐顯示
		mp_bar.show_percentage = false
	if mp_label:
		mp_label.text = "%d / %d" % [current_mp, mp_bar.max_value]
...
func _find_ui_nodes() -> void:
	...
	if scene_root != null:
		...
		mp_bar = scene_root.get_node_or_null("CanvasLayer2/ChargeBar") as ProgressBar
		mp_label = scene_root.get_node_or_null("CanvasLayer2/ChargeBar/Label") as Label
	...
	if mp_bar == null: mp_bar = get_node_or_null("CanvasLayer2/ChargeBar") as ProgressBar
	if mp_label == null: mp_label = get_node_or_null("CanvasLayer2/ChargeBar/Label") as Label
...
func _get_mp_text() -> String:
	var max_m = Playerdata_Globle.max_mp if "max_mp" in Playerdata_Globle else 100.0
	return str(int(current_mp)) + " / " + str(int(max_m))
...
func _arrange_headshot_bullets() -> void:
	...
	var total_width = float(num_bullets - 1) * bullet_spacing_head
	for i in range(num_bullets):
		var child = head_bullet_display.get_child(i) as Sprite2D
		var target_x = -total_width / 2.0 + float(i) * bullet_spacing_head
...
func handle_aim_and_release() -> void:
	# 如果沒有子彈可射，或者「魔力不足」，就不能進瞄準狀態
	if captured_bullets.is_empty() or current_mp < skill_mp_cost:
		if is_aiming:
			is_aiming = false
...
func use_stamina_potion() -> void:
	if Playerdata_Globle.stamina_potion > 0 and current_stamina < Playerdata_Globle.max_stamina:
		...
		current_stamina = minf(current_stamina + 50.0, Playerdata_Globle.max_stamina)
		print("使用了體力瓶，剩餘: ", Playerdata_Globle.stamina_potion)

func use_mp_potion() -> void:
	var max_m = Playerdata_Globle.max_mp if "max_mp" in Playerdata_Globle else 100.0
	if Playerdata_Globle.mp_potion > 0 and current_mp < max_m:
		Playerdata_Globle.mp_potion -= 1
		current_mp = minf(current_mp + 50.0, max_m) # 假設回復 50 魔力
		_update_player_ui()
