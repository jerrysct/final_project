...
@onready var head_bullet_display: Node2D = get_node_or_null("HeadBulletDisplay") as Node2D

# --- 道具 UI 標籤引用 ---
var btn_hp_label: Label = null
var btn_stamina_label: Label = null
var btn_mp_label: Label = null
var btn_invincible_label: Label = null
...
# --- 控制「玩家頭上子彈圖示」的顯示規格 ---
@export var head_icon_pixel_size: float = 24.0
@export var bullet_spacing_head: float = 30.0
...
func handle_movement(direction: Vector2) -> void:
	if Input.is_action_just_pressed("dash") and direction != Vector2.ZERO and (has_infinite_stamina or current_stamina >= Playerdata_Globle.dash_stamina_cost) and dash_cd_timer <= 0.0:
		perform_dash()
