...
@onready var attack_director: Node = get_node_or_null("Boss2AttackDirector")
...
func _ready() -> void:
	...
	if attack_director != null and attack_director.has_method("setup"):
		attack_director.setup(self)
	else:
		if debug_enabled:
			print("Boss2AttackDirector 不存在，或沒有 setup() 方法")
...
func _physics_process(delta: float) -> void:
	...
	if attack_director != null and attack_director.has_method("process_attack"):
		await attack_director.process_attack(delta)
	...
	while attempts < attack_reroll_max_attempts:
		...
		if attack_name == "special_bullet":
			if _can_use_special_bullet():
				...
				if special_bullet_patterns != null and special_bullet_patterns.has_method("execute_random_pattern"):
...
func enter_weak_state() -> void:
	...
	var duration: float = weak_duration
